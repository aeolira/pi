...
2. 定义包含所有必需颜色的主题（参见[颜色 Token](#颜色-token)）：
...
**热重载：** 编辑当前激活的自定义主题文件时，pi 自动重载以提供即时的视觉反馈。
...
**浅色终端：** 使用较暗、柔和的颜色和较低的对比度。

**颜色协调：** 从基础调色板（Nord、Gruvbox、Tokyo Night）开始，在 `vars` 中定义，并一致引用。

**测试：** 用不同的消息类型、工具状态、Markdown 内容和长文本换行检查你的主题。
